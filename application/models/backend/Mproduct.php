...
class Mproduct extends CI_Model {
    ...
    public function product_sanpham($search = '', $limit, $first)
    {
		if($search != '') {
	...
	public function productSearch($key, $limit, $first)
	{
	...
	public function product_count($key)
    {
		$this->db->like('name', $key);
        $this->db->where('status', 1);
		// echo $this->db->select();die;
        $query = $this->db->get($this->table);
        return count($query->result_array());
    }

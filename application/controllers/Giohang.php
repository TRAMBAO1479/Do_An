...
class Giohang extends CI_Controller {
	...
	public function index(){
        $this->data['title']='Sun And Flower - Giỏ hàng của bạn';
    ...
    public function checkout(){
        if($this->session->userdata('session24hStore')){
            ...
            if ($this->form_validation->run() == TRUE) {
                ...
                $this->data['title']='Sun And Flower - Đặt hàng';
    ...
    public function info_order(){
        ...
        if($this->form_validation->run() == TRUE){
            ...
            $config['smtp_user']    = 'sale.sunandflower@gmail.com';
            ...
            $this->email->from('sunandflower@gmail.com', 'Sun And Flower');
            $list = array('ngotrungphat@gmail.com');//Mặc định là email của admin
            $this->email->to($list);
            $this->email->subject('Sun And Flower');
            ...
            $this->data['title']='Sun And Flower - Thông tin đơn hàng';
    ...
    public function thankyou(){
        if($this->session->userdata('info-customer')||$this->session->userdata('session24hStore')){
            ...
            $this->email->to($list);
            $this->email->subject('Cửa hàng Sun And Flower');
            $body = $this->load->view('frontend/modules/email',$data,TRUE);
            $this->email->message($body); 
            $this->email->send();
        }   
        $this->data['title']='Sun And Flower - Kết quả đơn hàng';

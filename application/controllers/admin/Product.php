...
class Product extends CI_Controller {
	...
	public function index(){		
		...
		$this->data['list']=$this->Mproduct->product_sanpham($search, $limit, $first);
	...
	public function update($id){
		...
		$this->data['row']['config_image'] = [
			'base_url' => './public/images/products/'
		];
		$d=getdate();
		$today=$d['year']."/".$d['mon']."/".$d['mday']." ".$d['hours'].":".$d['minutes'].":".$d['seconds'];
		$this->load->library('form_validation');
		...
		if ($this->form_validation->run() == TRUE){
			...
			$mydata['config_image'] = $config['upload_path'];
			...
			if(isset($_FILES['avatar'])) {
				...
				$config['max_size']             = 4000;
				// $this->load->library('upload', $config);
				$this->load->set_upload_path($config['upload_path']);
				// $this->load->set_filename($config['upload_path']);
				if ( $this->upload->do_upload('img')){
					$data = $this->upload->data();
